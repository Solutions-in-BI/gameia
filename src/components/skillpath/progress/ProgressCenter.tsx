/**
 * Centro de Progresso Unificado
 * Dashboard central com todo o avanço do usuário:
 * - Nível, XP, Moedas
...
type ProgressTab = "overview" | "skills" | "achievements";

export function ProgressCenter() {
  const { profile, isAuthenticated } = useAuth();
  const { level, xp, progress: levelProgress, levelInfo } = useLevel();
  const { getProgress, getAchievementsWithStatus, stats } = useAchievements();
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-muted-foreground max-w-md mb-6">
          Faça login para acompanhar sua evolução, skills, conquistas e todo seu
          avanço na plataforma.
        </p>
        <button className="btn-primary-gameia">Criar minha conta</button>
      </div>
    );
  }
            ...
            <div className="gameia-card p-6">
              ...
              <div className="h-3 bg-muted rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(unlockedSkills.length / Math.max(skills.length, 1)) * 100}%`,
                  }}
                />
              </div>
            </div>
